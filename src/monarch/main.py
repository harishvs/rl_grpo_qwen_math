...
async def main(config_path: str):
    ...
    for step in range(max_steps):
        ...
        # 7. Sync weights via NCCL all_gather (every N steps)
        if step > 0 and step % config.trainer.weight_sync_interval == 0:
            weights_list = await learner.gather_weights_nccl.call()
            weights_bytes = weights_list.values()[0]  # rank 0's gathered bytes
